...
def validate_heightfield(field: HeightField) -> dict[str, Any]:
    ...
    if any(not isinstance(value, (int, float)) or not math.isfinite(float(value)) for row in field.values for value in row):
        errors.append("heightfield contains NaN/non-numeric values")
    if field.nodata is not None and any(value == field.nodata for row in field.values for value in row):
        errors.append("heightfield contains unresolved nodata samples")
