...
def generate_baker_v04(output_dir: Path = OUTPUT_DIR) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "qa": report["qa"],
        "inputs": {
            "spec": {"path": str(SPEC_PATH.relative_to(ROOT)).replace("\\", "/"), "sha256": semantic_json_sha256(SPEC_PATH)},
            "referenceProfile": {"path": str(REFERENCE_PATH.relative_to(ROOT)).replace("\\", "/"), "sha256": semantic_json_sha256(REFERENCE_PATH)},
            "featureSnapshot": {"path": str(SNAPSHOT_PATH.relative_to(ROOT)).replace("\\", "/"), "sha256": semantic_json_sha256(SNAPSHOT_PATH)},
        },
